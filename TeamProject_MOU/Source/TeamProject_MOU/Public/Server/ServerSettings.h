...
public:
	 ...
	 *
	 * [기본이 false 인 이유]
	 *   우리 팀은 **수동 포트포워딩**으로 운영하기로 했다. 그렇게 정한 이상
	 *   UPnP 시도는 이득이 없고 손해만 있다:
	 *
	 *     · 방 만들기 창이 열릴 때마다 SSDP 탐색으로 최대 3초를 버린다.
	 *       공유기가 UPnP 를 끄고 있으면 그 3초는 100% 낭비다.
	 *     · "공유기에 포트를 여는 중입니다..." 가 떴다가 실패하므로,
	 *       실제로는 수동 포워딩으로 잘 되는 상황인데도 뭔가 잘못된 것처럼 보인다.
	 *
	 * [켜야 할 때]
	 *   팀원 공유기에 손댈 수 없는 상황(남의 집, 학교 망 등)에서 방장을 해야 하면
	 *   켠다. 그때도 CGNAT 이면 UPnP 로는 못 푼다 — SERVER_INTEGRATION.md 14-5절.
	 *
	 * [★ 이 값과 무관하게 반드시 필요한 것]
	 *   방장이 되려면 **그 PC 가 있는 네트워크의 공유기**에
	 *     외부 UDP 7777 -> 방장 PC 의 LAN IP : 7777
	 *   이 열려 있어야 한다. UPnP 는 그것을 자동으로 해주는 수단일 뿐이고,
	 *   이 설정을 끈다는 것은 "그 일을 사람이 직접 한다" 는 뜻이다.
	 *   서버 쪽 공유기에 넣은 포워딩은 이 경로와 아무 상관이 없다 —
	 *   참여자의 게임 트래픽은 서버를 거치지 않고 방장에게 직접 간다.
	 *
	 * 실행 인자로 이번 실행만 켤 수 있다:  -MOUUseUpnp=1
	 */
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, Category = "MOU|Lobby")
